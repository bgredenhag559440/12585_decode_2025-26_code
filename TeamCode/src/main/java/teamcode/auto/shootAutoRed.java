...
public class shootAutoRed extends LinearOpMode { // Renamed class for clarity
    ...
    public void runOpMode() {
        ...
        spinner.setPosition(0.65);

        sleep(1000);

        //turn on shooters
        leftShoot.setPower(0.694);
        rightShoot.setPower(-0.694);

        sleep(2000);

        //prepare and shoot spinner arm
        spinner.setPosition(.4);
    ...
    public double[] strafe(String direction, double timeoutS) {
        ...
        } else if (direction.equalsIgnoreCase("left")) { //right does not work bc one of the wheels doesn't turn on the ground i think
            values = new double[]{-1.0, 1.0, 1.0, -1.0, timeoutS};
        }
        return values;
    }
