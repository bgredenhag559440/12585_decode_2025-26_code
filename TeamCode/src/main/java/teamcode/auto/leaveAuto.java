...
//@Disabled
public class leaveAuto extends LinearOpMode { // Renamed class for clarity
    ...
    public void runOpMode() {
        ...
        gate.setPosition(1);
        //encoderDrive(STRAFE_SPEED, strafe("left", 0.6));
        encoderDrive(TURN_SPEED, turn("right", 0.5)); //right front is previously negative so its a double negative which is why its put as negative but it acts like its positive
...
//                telemetry.addData("Runtime: ", "%7d", runtime.seconds());
    ...
    public void encoderDrive(double speed, double[] strafeOrTurnPatterns) {
        int leftFrontTarget;
        int leftRearTarget;
        int rightFrontTarget;
        int rightRearTarget;
        ...
        if (opModeIsActive()) {
            ...
            // Determine new target position, and pass to motor controller
            leftFrontTarget = leftFrontDrive.getCurrentPosition() + (int) (strafeOrTurnPatterns[0] * COUNTS_PER_INCH);
            leftRearTarget = leftRearDrive.getCurrentPosition() + (int) (strafeOrTurnPatterns[1] * COUNTS_PER_INCH);
            rightFrontTarget = rightFrontDrive.getCurrentPosition() + (int) (strafeOrTurnPatterns[2] * COUNTS_PER_INCH);
            rightRearTarget = rightRearDrive.getCurrentPosition() + (int) (strafeOrTurnPatterns[3] * COUNTS_PER_INCH);
            ...
            // keep looping while we are still active, there is time left, and ALL motors are running to position.
            // Using `isBusy()` is the correct way to check if RUN_TO_POSITION has completed.
            while (opModeIsActive() && (runtime.seconds() < strafeOrTurnPatterns[4]) &&
                    (leftFrontDrive.isBusy() && leftRearDrive.isBusy() &&
                            rightFrontDrive.isBusy() && rightRearDrive.isBusy()))
            {
                // Display current drive motor and odometry pod positions for the driver.
